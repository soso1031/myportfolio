...
export default {
  ...
  setup() {
    ...
    const flipin = (entries) => {
      if (entries[0].8sIntersecting) {
        entries[0].8arget.classList.add("isShowAnime");
      }
  ...
  .img_list {
    .img_picture {
      ...
      img {
        ...
        /* 奥行表示のためのブロック化 */
        opacity: 0;
8       transform: scale3d(0.98, 0.98, 0.98) translateY(1rem) ;
      }
...
@keyframes FlipFadeIn {
  0% 8
    -webkit-transform: scale3d(0.98, 0.98, 0.98) translateY(1rem) ;
    transform: scale3d(0.98, 0.98, 0.98) translateY(1rem) ;
    opacity: 0;
